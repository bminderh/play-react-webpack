...
class AuthenticationController @Inject() (
  ...
  def notFoundDefault(implicit request: RequestHeader) = Future.successful(NotFound)
        ...
        userService.retrieve(loginInfo).flatMap {
          ...
          case None => {
            ...
            for {
              ...
              mailer.welcome(savedUser, link = routes.AuthenticationController.handleStartSignUp.absoluteURL())
              Ok
            }
          }
        }
          ...
          case Some(user) => {
            env.authenticatorService.create(user.email).flatMap { authenticator =>
              ...
              for {
                ...
                result <- env.authenticatorService.embed(cookie, Ok)
              } yield {
    ...
    signInForm.bindFromRequest.fold(
      ...
      formData => {
        ...
        credentialsProvider.authenticate(Credentials(identifier, password)).flatMap { loginInfo =>
          ...
          case e: ProviderException => Redirect(routes.AuthenticationController.authenticate)
        }
      }
    )
  }
  ...
  private lazy val rememberMeParams: (FiniteDuration, Option[FiniteDuration], Option[FiniteDuration]) = {
    ...
    (
      cfg.getConfig("authenticatorExpiry").asInstanceOf[FiniteDuration],
      Some(cfg.getConfig("authenticatorIdleTimeout").asInstanceOf[FiniteDuration]),
      Some(cfg.getConfig("cookieMaxAge").asInstanceOf[FiniteDuration])
    )
  }
      ...
      email => userService.retrieve(email).flatMap {
        case Some(_) => {
          ...
          tokenService.create(token).map { _ =>
            mailer.forgotPassword(email, link = routes.AuthenticationController.handleStartSignUp.absoluteURL())
            Ok
          }
        }
  ...
  ) verifying ("auth.passwords.notequal", passwords => passwords._2 == passwords._1))
            ...
            userService.retrieve(loginInfo).flatMap {
              case Some(user) => {
                for {
                  ...
                  result <- env.authenticatorService.renew(authenticator, Ok)
                } yield {
                  tokenService.consume(tokenId)
                  env.eventBus.publish(LoginEvent(user, request))
                  result
  ...
  val changePasswordForm = Form(tuple(
    ...
    "password1" -> passwordValidation,
    "password2" -> nonEmptyText
  ) verifying ("auth.passwords.notequal", passwords => passwords._3 == passwords._2))
      ...
      passwords => {
        credentialsProvider.authenticate(Credentials(request.identity.email, passwords._1)).flatMap { loginInfo =>
          for {
            ...
            result <- env.authenticatorService.renew(authenticator, Redirect(routes.Application.index))
          } yield result
        }.recover {
          case e: ProviderException => BadRequest
        }
      }
    )
  }
